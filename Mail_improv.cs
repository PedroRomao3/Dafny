// Dafny program Mail_improv.dfy compiled into C#
...
// Command-line arguments: run --no-verify /home/jmffelisberto/Desktop/mfs/dafny/Dafny/Mail_improv.dfy
// Mail_improv.dfy

function InsertAt<T>(l: L.List<T>, p: nat, x: T): L.List<T>
  requires p <= L.len(l)
  ensures L.len(InsertAt(l, p, x)) == L.len(l) + 1
  ensures L.elementSeq(InsertAt(l, p, x)) == L.elementSeq(L.take(l, p)) + [x] + L.elementSeq(L.drop(l, p))
  decreases l, p
{
  if p == 0 then
    Cons(x, l)
  else
    match l case Cons(h, t) => Cons(h, InsertAt(t, p - 1, x)) case Nil() => Nil
}
...
lemma /*{:_inductionTrigger _default.remove(l, x)}*/ /*{:_induction l}*/ RemovePreservesExclusion<T>(l: List<T>, x: T, y: T)
...
lemma /*{:_inductionTrigger remove(l, x)}*/ /*{:_induction l, x}*/ RemovePreservesSystemBoxes(l: List<Mailbox>, x: Mailbox, i: Mailbox, d: Mailbox, t: Mailbox, s: Mailbox, a: Mailbox)
  requires i !in ListElements(l)
  requires d !in ListElements(l)
  requires t !in ListElements(l)
  requires s !in ListElements(l)
  requires a !in ListElements(l)
...
method test()
{
  ...
  assert ma.inbox.messages == ma.drafts.messages == ma.trash.messages == ma.sent.messages == {};
  ma.newMailbox(""students"");
  assert exists mb: Mailbox {:trigger mb.messages} {:trigger mb.name} {:trigger mb in ma.userBoxes} :: mb in ma.userBoxes && mb.name == ""students"" && mb.messages == {};
  var s := new Address(""email@address.com"");
  ma.newMessage(s);
  assert exists nw: Message {:trigger {nw}} :: ma.drafts.messages == {nw};
}
...
class Message {
  ...
  method addRecipient(position: nat, recipient: Address)
    requires position <= L.len(recipients)
    modifies this
    ensures L.len(recipients) == L.len(old(recipients)) + 1
    ensures L.elementSeq(recipients) == L.elementSeq(L.take(old(recipients), position)) + [recipient] + L.elementSeq(L.drop(old(recipients), position))
    ensures {id, date, sender} == old({id, date, sender})
    ensures content == old(content)
    decreases position, recipient
  {
...
class MailApp {
  ...
  method newMailbox(n: string)
    requires isValid()
    requires n != """"
    requires forall mb: Mailbox {:trigger mb.name} {:trigger mb in userBoxes} :: mb in userBoxes ==> mb.name != n
    modifies this
    ensures isValid()
    ensures exists mb: Mailbox {:trigger mb.messages} {:trigger mb.name} {:trigger mb in userBoxes} :: mb in userBoxes && mb.name == n && mb.messages == {}
    ensures [inbox, drafts, trash, sent, archive] == old([inbox, drafts, trash, sent, archive])
  ...
  method newMessage(s: Address)
    ...
    ensures isValid()
    ensures exists m: Message {:trigger m.sender} {:trigger m in drafts.messages} :: m in drafts.messages && m.sender == s && fresh(m)
    ensures |drafts.messages| == |old(drafts.messages)| + 1
    ensures forall m: Message {:trigger m in drafts.messages} {:trigger m in old(drafts.messages)} :: m in old(drafts.messages) ==> m in drafts.messages
    ensures old(drafts.messages) == {} ==> exists m: Message {:trigger {m}} :: drafts.messages == {m}
    decreases s
  {
    var m := new Message(s);
    drafts.add(m);
  ...
  method moveMessage(m: Message, fromMailbox: Mailbox, toMailbox: Mailbox)
    ...
    modifies this, m, fromMailbox, toMailbox
    ensures isValid()
    ...
    decreases m, fromMailbox, toMailbox
  {
    fromMailbox.remove(m);
    m.setLastState(fromMailbox);
    toMailbox.add(m);
  }
  ...
  method unarchiveMessage(m: Message)
    requires isValid()
    requires m.lastState != archive
    requires m.lastState != null
    modifies this, m, archive, m.lastState
    ensures isValid()
    decreases m
  {
    moveMessage(m, archive, m.lastState);
    m.setLastState(archive);
  }
...
module List {
    ...
    decreases l
  {
    ...
    case Nil() =>
      0
    case Cons(h, t) =>
      (if h == x then 1 else 0) + count(t, x)
  }
  ...
  function remove<T(==)>(l: List<T>, x: T): List<T>
    ensures var l': List<T> := remove(l, x); x !in elements(l') && elements(l') == elements(l) - {x}
    decreases l
  {
    match l
    case Nil() =>
...
namespace List {
  ...
  public partial class __default {
    public static BigInteger len<__T>(List._IList<__T> l) {
      BigInteger _0___accumulator = BigInteger.Zero;
    TAIL_CALL_START: ;
      List._IList<__T> _source0 = l;
      {
        if (_source0.is_Nil) {
          return BigInteger.Zero;
        }
      ...
      {
        List._IList<__T> _1_t = _source0.dtor_tail;
        return (BigInteger.One) + (List.__default.len<__T>(_1_t));
      }
    }
    public static BigInteger count<__T>(List._IList<__T> l, __T x)
    {
      BigInteger _0___accumulator = BigInteger.Zero;
    TAIL_CALL_START: ;
      List._IList<__T> _source0 = l;
      ...
      {
        __T _1_h = _source0.dtor_head;
        List._IList<__T> _2_t = _source0.dtor_tail;
        return (((object.Equals(_1_h, x)) ? (BigInteger.One) : (BigInteger.Zero))) + (List.__default.count<__T>(_2_t, x));
      }
    }
    public static __T at<__T>(List._IList<__T> l, BigInteger i)
    {
    TAIL_CALL_START: ;
      if ((i).Sign == 0) {
        return (l).dtor_head;
      } else {
        List._IList<__T> _in0 = (l).dtor_tail;
    ...
    public static List._IList<__T> append<__T>(List._IList<__T> l1, List._IList<__T> l2)
    {
      List._IList<__T> _source0 = l1;
      {
        if (_source0.is_Nil) {
          return l2;
        }
      }
      {
        ...
        List._IList<__T> _1_t = _source0.dtor_tail;
        return List._List<__T>.create_Cons(_0_h, List.__default.append<__T>(_1_t, l2));
      }
    }
    public static List._IList<__T> take<__T>(List._IList<__T> l, BigInteger n)
    {
      if ((n).Sign == 0) {
        return List._List<__T>.create_Nil();
      } else {
        return List._List<__T>.create_Cons((l).dtor_head, List.__default.take<__T>((l).dtor_tail, (n) - (BigInteger.One)));
      }
    }
    public static List._IList<__T> drop<__T>(List._IList<__T> l, BigInteger n)
    {
    TAIL_CALL_START: ;
      if ((n).Sign == 0) {
        return l;
    ...
    public static List._IList<__T> @remove<__T>(List._IList<__T> l, __T x)
    {
      List._IList<__T> _source0 = l;
      {
        if (_source0.is_Nil) {
      ...
      }
      {
        __T _0_h = _source0.dtor_head;
        List._IList<__T> _1_t = _source0.dtor_tail;
        if (object.Equals(x, _0_h)) {
          return List.__default.@remove<__T>(_1_t, x);
        } else {
          return List._List<__T>.create_Cons(_0_h, List.__default.@remove<__T>(_1_t, x));
        }
      }
    }
  }
  ...
  public interface _IList<T> {
    ...
    bool is_Cons { get; }
    T dtor_head { get; }
    List._IList<T> dtor_tail { get; }
    _IList<__T> DowncastClone<__T>(Func<T, __T> converter0);
  }
  public abstract class _List<T> : _IList<T> {
    public _List() {
    }
    public static List._IList<T> Default() {
      return create_Nil();
    }
    public static Dafny.TypeDescriptor<List._IList<T>> _TypeDescriptor() {
      return new Dafny.TypeDescriptor<List._IList<T>>(List._List<T>.Default());
    }
    public static _IList<T> create_Nil() {
      return new List_Nil<T>();
    }
    public static _IList<T> create_Cons(T head, List._IList<T> tail) {
    ...
    public List._IList<T> dtor_tail {
      get {
        var d = this;
  ...
  public class List_Nil<T> : _List<T> {
    ...
    public override bool Equals(object other) {
      var oth = other as List._List_Nil<T>;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "List.List.Nil";
      return s;
    }
  }
  public class List_Cons<T> : _List<T> {
    public readonly T _head;
    public readonly List._IList<T> _tail;
    public List_Cons(T head, List._IList<T> tail) : base() {
      this._head = head;
      this._tail = tail;
    }
    public override _IList<__T> DowncastClone<__T>(Func<T, __T> converter0) {
    ...
    public override bool Equals(object other) {
      var oth = other as List._List_Cons<T>;
...
namespace _module {
  ...
  public partial class __default {
    public static List._IList<__T> InsertAt<__T>(List._IList<__T> l, BigInteger p, __T x)
    {
      if ((p).Sign == 0) {
        return List._List<__T>.create_Cons(x, l);
      } else {
        List._IList<__T> _source0 = l;
        {
          if (_source0.is_Cons) {
            __T _0_h = _source0.dtor_head;
            List._IList<__T> _1_t = _source0.dtor_tail;
            return List._List<__T>.create_Cons(_0_h, __default.InsertAt<__T>(_1_t, (p) - (BigInteger.One), x));
          }
        }
        {
          return List._List<__T>.create_Nil();
    ...
    {
      MailApp _0_ma;
      MailApp _nw0 = new MailApp();
      _nw0.__ctor();
      _0_ma = _nw0;
      (_0_ma).newMailbox(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("students"));
      Address _1_s;
      Address _nw1 = new Address();
      _nw1.__ctor(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("email@address.com"));
      _1_s = _nw1;
      (_0_ma).newMessage(_1_s);
    }
  }

  public partial class Address {
    public Address() {
      this.@value = Dafny.Sequence<Dafny.Rune>.Empty;
    }
    public Dafny.ISequence<Dafny.Rune> @value {get; set;}
    public void __ctor(Dafny.ISequence<Dafny.Rune> v)
    {
      (this).@value = v;
    }
  }
  ...
  public partial class Date {
    ...
    public void __ctor(BigInteger t)
    {
      (this).timestamp = t;
    }
  }
  ...
  public partial class Message {
    public Message() {
      this.id = default(MessageId);
      this.content = Dafny.Sequence<Dafny.Rune>.Empty;
      this.date = default(Date);
      this.sender = default(Address);
      this.recipients = List._List<Address>.Default();
      this.lastState = ((Mailbox)null);
    }
    public MessageId id {get; set;}
    public Dafny.ISequence<Dafny.Rune> content {get; set;}
    public Date date {get; set;}
    public Address sender {get; set;}
    public List._IList<Address> recipients {get; set;}
    public Mailbox lastState {get; set;}
    public void __ctor(Address s)
    {
      MessageId _nw0 = new MessageId();
      _nw0.__ctor(BigInteger.Zero);
      (this).id = _nw0;
      Date _nw1 = new Date();
      _nw1.__ctor(BigInteger.Zero);
      (this).date = _nw1;
      (this).content = Dafny.Sequence<Dafny.Rune>.UnicodeFromString("");
      (this).sender = s;
      (this).recipients = List._List<Address>.create_Nil();
      (this).lastState = (Mailbox)null;
    }
    public void setContent(Dafny.ISequence<Dafny.Rune> c)
    {
      (this).content = c;
    }
    ...
    public void addRecipient(BigInteger position, Address recipient)
    {
      (this).recipients = __default.InsertAt<Address>(this.recipients, position, recipient);
    }
    public void setLastState(Mailbox mb)
    {
      (this).lastState = mb;
    }
  }
  ...
  public partial class Mailbox {
    public Mailbox() {
      ...
      this.messages = Dafny.Set<Message>.Empty;
    }
    public Dafny.ISequence<Dafny.Rune> name {get; set;}
    public Dafny.ISet<Message> messages {get; set;}
    public void __ctor(Dafny.ISequence<Dafny.Rune> n)
    {
      (this).name = n;
      (this).messages = Dafny.Set<Message>.FromElements();
    }
    public void @add(Message m)
    {
      (this).messages = Dafny.Set<Message>.Union(this.messages, Dafny.Set<Message>.FromElements(m));
    }
    public void @remove(Message m)
    {
      (this).messages = Dafny.Set<Message>.Difference(this.messages, Dafny.Set<Message>.FromElements(m));
    }
    public void empty()
    {
      (this).messages = Dafny.Set<Message>.FromElements();
  ...
  public partial class MailApp {
    public MailApp() {
      this.inbox = default(Mailbox);
      this.drafts = default(Mailbox);
      this.trash = default(Mailbox);
      this.sent = default(Mailbox);
      this.archive = default(Mailbox);
      this.userboxList = List._List<Mailbox>.Default();
    }
    public Mailbox inbox {get; set;}
    public Mailbox drafts {get; set;}
    public Mailbox trash {get; set;}
    public Mailbox sent {get; set;}
    public Mailbox archive {get; set;}
    public List._IList<Mailbox> userboxList {get; set;}
    public void __ctor()
    {
      Mailbox _nw0 = new Mailbox();
      _nw0.__ctor(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Inbox"));
      (this).inbox = _nw0;
      Mailbox _nw1 = new Mailbox();
      _nw1.__ctor(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Drafts"));
      (this).drafts = _nw1;
      Mailbox _nw2 = new Mailbox();
      _nw2.__ctor(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Trash"));
      (this).trash = _nw2;
      Mailbox _nw3 = new Mailbox();
      _nw3.__ctor(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Sent"));
      (this).sent = _nw3;
      Mailbox _nw4 = new Mailbox();
      _nw4.__ctor(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Archive"));
      (this).archive = _nw4;
      (this).userboxList = List._List<Mailbox>.create_Nil();
    }
    public void deleteMailbox(Mailbox mb)
    {
      (this).userboxList = List.__default.@remove<Mailbox>(this.userboxList, mb);
      (mb).empty();
    }
    public void newMailbox(Dafny.ISequence<Dafny.Rune> n)
    {
      Mailbox _0_mb;
      Mailbox _nw0 = new Mailbox();
      _nw0.__ctor(n);
      _0_mb = _nw0;
      (this).userboxList = List._List<Mailbox>.create_Cons(_0_mb, this.userboxList);
    }
    public void newMessage(Address s)
    {
      Message _0_m;
      Message _nw0 = new Message();
      _nw0.__ctor(s);
      _0_m = _nw0;
      (this.drafts).@add(_0_m);
    }
    public void moveMessage(Message m, Mailbox fromMailbox, Mailbox toMailbox)
    {
      (fromMailbox).@remove(m);
      (m).setLastState(fromMailbox);
      (toMailbox).@add(m);
    }
    public void deleteMessage(Message m, Mailbox mb)
    {
    ...
    public void archiveMessage(Message m, Mailbox mb)
    {
      (this).moveMessage(m, mb, this.archive);
    }
    public void unarchiveMessage(Message m)
    {
      (this).moveMessage(m, this.archive, m.lastState);
      (m).setLastState(this.archive);
    }
