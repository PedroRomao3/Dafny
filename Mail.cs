// Dafny program Mail.dfy compiled into C#
...
// Command-line arguments: test --no-verify /home/pedroromao/dafny_proj/Dafny/Mail.dfy
// Mail.dfy
  ...
  decreases l, p
{
  if p == 0 then
    ...
    match l case Cons(h, t) => Cons(h, InsertAt(t, p - 1, x)) case Nil() => L.Nil
}
...
lemma /*{:_inductionTrigger _default.remove(l, x)}*/ /*{:_induction l}*/ RemovePreservesExclusion<T>(l: L.List<T>, x: T, y: T)
  ensures y !in L.elements(l) ==> y !in L.elements(L.remove(l, x))
  decreases l
{
  match l
...
lemma /*{:_inductionTrigger _default.remove(l, x)}*/ /*{:_induction l, x}*/ RemovePreservesSystemBoxes(l: L.List<Mailbox>, x: Mailbox, i: Mailbox, d: Mailbox, t: Mailbox, s: Mailbox)
  requires i !in ListElements(l)
  requires d !in ListElements(l)
  requires t !in ListElements(l)
  requires s !in ListElements(l)
  ensures i !in ListElements(L.remove(l, x))
  ensures d !in ListElements(L.remove(l, x))
  ensures t !in ListElements(L.remove(l, x))
  ensures s !in ListElements(L.remove(l, x))
  decreases l, x, i, d, t, s
{
...
class Message {
  ...
  method addRecipient(p: nat, r: Address)
    requires p <= L.len(recipients)
    modifies this
    ensures L.len(recipients) == L.len(old(recipients)) + 1
    ensures L.elementSeq(recipients) == L.elementSeq(L.take(old(recipients), p)) + [r] + L.elementSeq(L.drop(old(recipients), p))
    ensures {id, date, sender} == old({id, date, sender})
    ensures content == old(content)
    decreases p, r
  {
...
class MailApp {
  ...
  var trash: Mailbox
  var sent: Mailbox
  var userboxList: L.List<Mailbox>
    ...
    decreases mb
  {
    ...
    userboxList := L.remove(userboxList, mb);
    userBoxes := ListElements(userboxList);
    mb.empty();
  }
  ...
  method newMessage(s: Address)
    ...
    ensures old(drafts.messages) == {} ==> |drafts.messages| == 1 && exists m: Message {:trigger m.sender} {:trigger {m}} :: drafts.messages == {m} && m.sender == s && fresh(m)
    decreases s
  {
    var m := new Message(s);
    drafts.add(m);
...
namespace _module {
  ...
  public partial class __default {
    ...
    public static void __Test____Main__(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> __noArgsParameter)
    {
      ...
      _0_success = true;
      if (!(_0_success)) {
        throw new Dafny.HaltException("Mail.dfy(9,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"Test failures occurred: see above.
").ToVerbatimString(false));}
  ...
  public partial class Message {
    ...
    public void addRecipient(BigInteger p, Address r)
    {
      (this).recipients = __default.InsertAt<Address>(this.recipients, p, r);
    }
  }
  ...
  public partial class Mailbox {
    ...
    {
      (this).messages = Dafny.Set<Message>.Union(Dafny.Set<Message>.FromElements(m), this.messages);
    }
  ...
  public partial class MailApp {
    ...
    public void moveMessage(Message m, Mailbox mb1, Mailbox mb2)
    {
      (mb1).@remove(m);
      (mb2).@add(m);
    }
